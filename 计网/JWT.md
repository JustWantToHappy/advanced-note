## JWT（JSON Web Token）
JWT就是一个字符串（身份令牌），三部分组成：header(json对象base64编码,记录了整个令牌的类型和签名算法).payload(用户信息对象base64编码，密码，身份证，手机号这些敏感信息不要存放在payload，可存用户id，以及角色).signature（前面两部分连接起来之后，按照头部固定的签名算法对整个令牌进行签名，一般通过一个密钥加密得到的字符串经过base64编码）
### header
```javascript
{
	"alg":"HS256",
