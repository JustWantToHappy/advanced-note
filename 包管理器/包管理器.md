...
### npm模块安装机制
1. npm会检查本地node_modules目录中是否已经安装过该模块，如果已经安装过，则不再重新安装
2. npm检查缓存中是否有相同的模块，如果有，直接从缓存中读取
3. 如果本地和缓存中都不存在，npm从registry指定的地址下载安装包，然后将其写入到本地的node_modules目录中，同时缓存起来
## pnpm
