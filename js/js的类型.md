> js是一门动态(动态指的是变量的类型是在运行时决定的，而不是在编译时决定的。)弱类型(弱类型指的是允许不同类型之间进行隐式类型转换。)的语言
...
- 隐式类型转换:四则运算，判断语句，`Native`调用，`JSON`方法 ###　显式转换
...
当不同数据类型进行相互计算，或者非布尔值转为布尔值时，会发生隐式类型转换

1. 自动转换为boolean值，比如if的条件语句，转换规则可以参考Boolean显式转换，一样的
2. 自动转换为string，js遇到预期应该是字符串的地方，就会将非字符串转换为字符串。具体规则是：先将复合类型的值转为原始类型的值，再将原始类型的值转换为字符串
3. js遇到预期未数字的地方，系统内部会自动调用Number方法
<br>
...
- 如果一边是字符串，则进行字符串拼接
- 如果两边都是其他类型（不是对象也不是number）,则都转换成number

> 需要注意的是-运算和+有些不同，如果一边是字符串，一边是number,则将string转换为number后运算
