...
- GBK编码：收录21003个汉字，包含国家标准GB13000-1中的全部
  中日韩汉字，和BIG5编码中的所有汉字。
- Unicode编码：国际标准字符集
- 图片数据存储：光学三原色：红绿蓝，简称RGB，写成十进制：(255,255,255)，也可以写成十六进制：FFFFFF。比如100*100像素的图片，每个像素点存储的都是RGB的值，看不同的图片格式，有的支持rgba
...
- 原码: 1000...0000  (-0，特殊值)
- 补码规定: 1000...0000 = -2^31 = -2147483648
...
- 堆空间：存储的是引用类型的空间
  - string串池：堆中的一块内存：当使用双引号直接赋值的时候，系统会直接检查该字符串在串池中是否存在，如果存在则复用，否则创建新对象
    - 静态存储位置（静态区）：静态变量是随着类的加载而加载的
