...
3. 子类重写父类方法时，返回值类型子类必须要小于等于父类，比如父类中返回值是Animal，子类方法返回值是Dog，这是合法的，如果父类方法中返回值是Number,而子类方法返回值中是Integer，这是不合法的
4. 建议：重写的方法尽量与父类保持一致
...
## 工具类
只包含静态方法的类，不创建实例，用于封装一组通用的，无状态的辅助功能，比如java.util.Arrays这个类，Arrays.fill方法等通用能力，通常工具类的构造方法是私有化的，防止实例化，也通常是final定义的，防止被继承
...
## 接口
- 接口使用interface定义:`public interface 接口名称{}`
- 接口不能实例化
- 接口和类之间是实现关系，通过implements关键字表示：`public class 类名 implements 接口名称{}`
- 接口的子类(实现类)：
		-	要么重写接口中的所有抽象方法
		- 要么是抽象类

注意1：接口的实现，可以单实现，也可以多实现：`public class 类名 implements 接口1,接口2{}`
注意2：实现类还可以在继承一个类的时候实现多个接口`public 类名 extends 父类 implements 接口名称1,接口名称2{}`
注意3：接口与接口之间是继承关系，可以单继承，也可以多继承
```java
interface CommonUtils{
    void append();
}

interface LogUtils extends CommonUtils{
    void print();
}
//细节：如果实现类实现了最下面的子接口，那么就需要重写所有的抽象方法
class LoginService implements LogUtils{
...
```java
interface Utils{
//   等同于 public static final double pi=Math.PI;
    double pi=Math.PI;
}
...
```java
interface Utils{
//  等同于 ：public abstract void swim();
    void swim();
}
```
- JDK8新特性：接口中可以定义有方法体的方法（作用：解决接口升级的问题）
	- 有方法体的方法是默认方法，必须使用default修饰
	- 接口中默认方法的定义格式：`public default 返回值类型 方法名(参数列表){}`，其中public可以省略，但是default必须写
	- 如果子类实现了多个接口，多个接口存在相同名字的默认方法，子类就必须重写这个方法
	- 允许接口中定义静态方法，格式：`public static 返回值类型 方法名称(参数列表){}`
...
class TableTennisPlayer  implements SpeakEnglish,SpeakChinese{
    public void sayEnglish(){

    }
    @Override
    public void say() {
	    //调用接口的say方法，因为SpeakEnglish和SpeakChinese都实现了say方法，所以前面要加上接口名，类似继承中子类调用父类的方法
        SpeakEnglish.super.say();
    }
}
```
- JDK9新特性：接口中可以定义私有方法
...
class MovePlaceCompany implements Transport{}

public class Main {
    public static void transport(Transport t){
        
    }

    public static void main(String[] args) {
        Car car = new Car();
        MovePlaceCompany move = new MovePlaceCompany();
        transport(car);
        transport(move);
    }
}
...
```
- 静态内部类
	-	只能访问外部类的静态变量和静态方法，如果想要访问非静态的需要创建外部类对象后访问（在静态内部类的方法中创建外部类实例）
	- 创建静态内部类的格式：外部类名.内部类名 变量名=new 外部类名.内部类名();
	- 调用静态方法的格式：外部类名.内部类名.方法名();
```java
class Outer{
    public static String name="外部类静态属性";
    static class Inner{
...
```
- 局部内部类：将内部类定义在方法里面就叫做局部内部类，类似方法中的局部变量，局部变量能用什么修饰符，局部内部类就能用哪些修饰符
```java
class Outer{
    public void say(){
        //static,public等关键字修饰符就不能使用
        final class Inner{
...
```
- 匿名内部类：本质上就是隐藏了名字的内部类，格式：`new 类名或者接口名(){}`
```java
interface Inter1{
    public abstract void say();
}
...
public class Main {
    public static void main(String[] args) {
        //将花括号{}包括内部的内容看做一个匿名内部类，如下这段代码就可以看作这个匿名内部类实现了Inter1这个接口
				//花括号前面如果是接口就是实现关系，如果是类就是继承关系
        new Inter1(){
            @Override
            public void say(){}
        };
...
```
外部类成员变量与内部类成员变量重名的时候，在内部类中如何访问？
```java
class Outer{
    private String name="外部类属性值";
    class Inner{
        String name="内部类属性值";
...
public class Main {
    public static void main(String[] args) {
        ...
        String result=inner.getOuterName();
